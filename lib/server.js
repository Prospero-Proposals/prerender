...
const BROWSER_CONNECTION_RETRY_ATTEMPTS = process.env.BROWSER_CONNECTION_RETRY_ATTEMPTS || 3;
const BROWSER_CONNECTION_RETRY_DELAY = process.env.BROWSER_CONNECTION_RETRY_DELAY || 2000;
...
server.init = function (options) {
	...
	this.options.browserConnectionRetryAttempts =
		this.options.browserConnectionRetryAttempts || BROWSER_CONNECTION_RETRY_ATTEMPTS;
	this.options.browserConnectionRetryDelay =
		this.options.browserConnectionRetryDelay || BROWSER_CONNECTION_RETRY_DELAY;
	this.options.chromeFlags = ['--no-sandbox', '--headless', '--disable-gpu', '--remote-debugging-port=9222', '--hide-scrollbars'];
...
server.waitForBrowserToConnect = function () {
	return new Promise((resolve, reject) => {
		var checks = 0;
		var maxRetries = this.options.browserConnectionRetryAttempts;
		var retryCount = 0;

		let check = () => {
			if (++checks > 100) {
				util.log(`Timed out waiting for ${this.browser.name} connection (attempt ${retryCount + 1}/${maxRetries})`);

				// Try to restart browser and reconnect
				if (retryCount < maxRetries) {
					retryCount++;
					checks = 0;

					util.log(`Attempting to restart ${this.browser.name} and reconnect...`);

					// Restart browser
					this.restartBrowser();

					// Wait a bit for browser to restart, then try to reconnect
					setTimeout(() => {
						this.startPrerender()
							.then(() => {
								util.log(`Successfully restarted ${this.browser.name}, retrying connection...`);
								check(); // Continue checking
							})
							.catch((err) => {
								util.log(`Failed to restart ${this.browser.name}:`, err);
								check(); // Continue checking anyway
							});
					}, this.options.browserConnectionRetryDelay);
				...
				return reject(`Timed out waiting for ${this.browser.name} connection after ${maxRetries} restart attempts`);
			}
...
server.onRequest = function (req, res) {
	...
	// Remove extra forward slashes from HTTPS URLs
	if (req.prerender.url && req.prerender.url.startsWith('https://')) {
		// Replace multiple consecutive slashes after https:// with a single slash
		req.prerender.url = req.prerender.url.replace(/https:\/\/+/, 'https://');
	}
	...
	this.firePluginEvent('requestReceived', req, res)
		...
		.catch((err) => {
			util.log(`Browser connection failed: ${err}`);
			req.prerender.statusCode = 503;
			req.prerender.statusCodeReason = 'Browser connection failed';
